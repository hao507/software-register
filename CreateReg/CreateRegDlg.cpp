...
{
	/*
		通过内联汇编获取到的cpuid与通过命令行获取的结果一致，
		在cmd中输入wmic cpu get processorid可获取cpu id
	*/
    //获取CPU序列号
    unsigned long s1, s2;
    ...
    __asm{
        mov eax, 01h	// 传入功能号
        xor edx, edx		// 清空结果,执行cpuid指令之后.结果会放在edx中
        cpuid				// 执行指令,获取结果
        mov s1, edx		// 读取结果
        mov s2, eax		// 读取结果
    }
    ...
    __asm{
        mov eax, 03h	// 传入功能号
        xor ecx, ecx		// 清空结果,执行cpuid指令之后.结果会放在edx中
        xor edx, edx	    // 同上
        cpuid			    // 执行指令
        mov s1, edx    // 读取结果
        mov s2, ecx    // 读取结果
...
{
	/*
		获取与指定根目录关联的文件系统的信息，这里的磁盘序列号是卷标号，
		不是硬盘序列号哦，格式化磁盘后磁盘序列号是会改变的
	*/
    DWORD ser;
    char namebuf[128];
    char filebuf[128];
    ...
    ::GetVolumeInformation("c:\\",		// 欲获取信息的磁盘的根路径
                           namebuf,				// 用于装载磁盘名的缓冲区
                           128,					    // 上面缓冲区的大小
                           &ser,						// 用于装载磁盘序列号的变量,其数据类型为LPDWORD，为指向DWORD类型的指针
                           0,							// 用于装载文件名每一部分的长度
                           0,							// 标志位
                           filebuf,					// 用于装载文件系统的名称
                           128						// 上面缓冲区的大小
